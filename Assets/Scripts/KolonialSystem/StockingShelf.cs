...
public class StockingShelf : PlayerInput.IShelfActions
{
    ...
    private const float TimeBetweenPlacements = 0.5f; // timer pause between placements
    private const float MoveSpeed = 3f;             // speed of placedPrefab flying to shelf
    ...
    public void Activate(PlayerInteract playerInteract)
    {
        ...
        _runner.StartCoroutine(StockingSequence(playerInteract));
    }
    ...
    private IEnumerator StockingSequence(PlayerInteract playerInteract)
    {
        ...
        arrowTransform.position = new Vector3(arrowTransform.position.x, arrowTransform.position.y + 1.95f, arrowTransform.position.z);
        ...
        while (_currentStockIndex < _stockingPositions.Count && _shelf.remainingStockCount > 0)
        {
            ...
            GameObject flyingItem = GameObject.Instantiate(_shelf.placedPrefab, boxTransform.position, Quaternion.identity);
    ...
    {
        // Reserved for future use (e.g. cancelling stocking early)
    }
